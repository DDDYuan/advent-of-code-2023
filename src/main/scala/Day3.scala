...
object Day3:
  private def load: List[String] = Source.fromResource("day3.csv").getLines().toList
  ...
  def part1: String =
    val input = load
    input.zipWithIndex.flatMap(toExtracted).filter(isPartNumber(_, input)).map(_._1).sum.toString
  def part2: String =
    val input = load
    input.zipWithIndex
      .flatMap(toExtracted)
